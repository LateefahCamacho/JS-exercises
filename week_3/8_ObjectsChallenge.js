...
// [true, false, undefined]
    const pluck = function(key, obj){
        return key.map(value => value[obj])
    } 
...
// [1, 4]
function minMaxKeyInObject(val) {
    // get all the keys as array 
    let keys = Object.keys(val);
    //parse a string argument and return an integer
    lowestKey = parseInt(keys[0]);
    highestKey = parseInt(keys[0]);
    //iterate over keys
    for (let i= 0; i <keys.length; i = i+1) {
      if ( parseInt(keys[i]) < lowestKey) {
        lowestKey = parseInt(keys[i]);
      }
      //if the current value greater than max
      if(parseInt(keys[i]) > highestKey) {
        highestKey = parseInt(keys[i]);
      }
    }
